...
project.path 	     <- "C:/Users/aihyv/OneDrive/Tiedostot/Covid/SEIR_SWE/"
...
figure.path        <- paste(project.path, "Results/", sep="")
...
#HUS <- read.table(file=paste(data.path,"/HUS.txt",sep=""), sep = "\t", header=TRUE)
#HUS<-HUS[-nrow(HUS),]
#HUS$COVID.19<- as.numeric(paste(HUS$COVID.19))
#HUS$pvm<-as.Date(HUS$pvm, format("%d.%m.%Y"))
...
url2 <- paste("https://www.ecdc.europa.eu/sites/default/files/documents/COVID-19-geographic-disbtribution-worldwide-",
              "2020-05-27", ".xlsx", sep = "")
try(GET(url2, authenticate(":", ":", type="ntlm"), write_disk(tf <- tempfile(fileext = ".xlsx"))))
tmp <- try(read_excel(tf))
tmp<-tmp[(tmp$geoId=="FI"),]
tmp<-tmp[tmp$dateRep>"2020-02-27",]
tmp<-tmp[order(tmp$dateRep),]
FI<-tmp[,c(1,5)]
FI$dateRep<-as.Date(FI$dateRep, format("%Y-%M-%D"))
#---------------------------------------------------------------------------------------------------
...
#p_symp = 0.5 #fraction reported cases. Denoted p_r in report. 
#p_asymp = 1-p_symp #fraction non-reported cases. Denoted p_o in report.
...
Rtol = 1e-5    #10
############################################################################################################
Estimate_function_Stockholm_only_local <- function(p_symp = 0.5, p_lower_inf = 0.5, gammaD = gammaD_value, 
                                                   eta = eta_value , iter = 20){     #iter=20
 ...
 # N<-1670000 #Uusimaa
  ...
  Incidence <- FI$cases #[-(1:5)]
  Datum     <- as.Date(FI$dateRep) #[-(1:5)]
  ...
  beta_decrease <- function(t, delta, epsilon, theta){
    ...
    t_b <- as.numeric(as.Date("2020-03-18")) - as.numeric(as.Date("2019-12-31")) # dagen för jobba hemma
  ...
  #You can instead choose the commented one 
  ...
  while(i <= iter){
    ...
    conl <- list(maxit = 100, parscale = c(0.0001, 1, 0.01), abstol = Atol, reltol = Rtol)
    #conl <- list(maxit = 100, parscale = c(0.000001, 1, 0.01)) 
...
plot(c(50:150),Basic_repr(c(50:150), delta = Est$par[1], epsilon = Est$par[2],  theta = Est$par[3]  ,gamma = gammaD),type="l", ylab="R0(t)",lwd=2, 
...
plot(c(50:150),beta(c(50:150), delta = Est$par[1], epsilon = Est$par[2],  theta = Est$par[3]),type="l", ylab="Infectivity",lwd=2, 
...
t   <- (Day[1]):(Day[length(Day)] + 45) # time in days
fit <- data.frame(ode(y = init, times = t, func = SEIR_model, parms = Opt_par))
...
dayatyear_march_may <- c(Day[1], 61, 61 + 14, 61 + 14 + 17, 61 + 14 + 17 + 14 , 61 + 14 + 17 + 14 + 16, 
                         61 + 14 + 17 + 14 + 16 + 14, 150)
...
plot(fit$time, fit[ , 4] + fit[ , 5], ylim = c(0, 12000 ) , type="l", col="red", 
...
MainTitle <- paste("Fitted SEIR model covid-19 Finland (whole country) \n Unreported cases ",round((1-p_symp_use)*100,digits=1),
